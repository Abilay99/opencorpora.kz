...
class SMSC(object):
    ...
    # format - формат сообщения (0 - обычное sms, 1 - flash-sms, 2 - wap-push, 3 - hlr, 4 - bin,
    # 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber, 11 - soc)
    ...
    def send_sms(self, phones, message, translit=0, time="", id=0, format=0, sender=False, query=""):
        formats = ["flash=1", "push=1", "hlr=1", "bin=1", "bin=2", "ping=1",
                   "mms=1", "mail=1", "call=1", "viber=1", "soc=1"]
        ...
        if SMSC_DEBUG:
            if m[1] > "0":
                print("Сообщение отправлено успешно. ID: " + m[0] + ", всего SMS: " + m[1] +
    ...
    def get_sms_cost(self, phones, message, translit=0, format=0, sender=False, query=""):
        ...
        m = self._smsc_send_cmd("send", "cost=1&phones=" + quote(phones) + "&mes=" + quote(message) +
                                ifs(sender is False, "", "&sender=" + quote(str(sender))) +
                                "&translit=" + str(translit) + ifs(format > 0, "&" + formats[format - 1], "") +
                                ifs(query, "&" + query, ""))
    ...
    # для HLR-запроса (<статус>, <время изменения>, <код ошибки sms>, <код IMSI SIM-карты>,
    # <номер сервис-центра>, <код страны регистрации>,
...
# smsc.send_sms("79999999999", "0605040B8423F0DC0601AE02056A0045C60C036D79736974652E72750001036D7973697465000101",
# format=5)
